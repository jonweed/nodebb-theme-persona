...
//	$(".navbar-fixed-top").autoHideNavbar();
	...
	function setupTeaserSlide() {
		require(['components'], function(components) {
			function toggle(ev) {
				...
				el=$target.parents('.ro-box:first').find('.ro-teaser');
				panel=$target.parents('.ro-panel-category-item:first');

				if (el.height() == 0) {
					el.animate({height: el.find('> .post-content').height()+'px'},'fast',function(){ $(this).css('height','auto'); });
	...
	function setupHoverCards() {
		...
		require(['components'], function(components) {
			components.get('topic').on('click', '.ro-toggle-vote', function(ev){
					...
					var panel=$(this).parents('.ro-panel-post:first'),el=panel.find('> .ro-box > .ro-drawer > .ro-vote-sm');
	...
	function generateUserCard(ev) {
		...
		var avatar = $(this),
				 post = avatar.parents('[component="post"]:first'),
				 panel=post.find('> .ro-panel-post'),
				 data = post.data('usercard');
