...
$(document).ready(function() {
	...
	$(window).on('action:ajaxify.end', function(ev, data) {
		...
		setupToggle('breadcrumb');

		switch(data.tpl_url) {
			case "categories":
				socket.on('event:new_post', updateCategoriesCount);
				setupToggle('categories/category','category/posts');
				break;

			case "category":
				setupToggle('category/topic');
				break;

			case "topic":
//				setupToggle('post','post/replies');
				break;
		}
	});
	...
	function setupToggle(component, filter) {
	  require(['components'], function(components) {
	    function toggle(ev) {
	      ...
	      var $target=$(ev.target).closest('[data-box]'),
	      block=$(ev.delegateTarget),
	      el=$target.closest('[component]').find('[box="'+$target.data('box')+'"]');

	      if (el.is(':visible')) {
	        block.trigger('close.slides');
	      }
	      else {
	        block.trigger('close.slides').one('close.slides',closeBox);
	        el.slideDown('fast');
	    ...
	    $('[component="'+component+'"]').on('click', (filter) ? '[component="'+filter+'"] [data-box]' : '[data-box]', toggle);
	  });
	}
	...
	function setupHoverCards() {
		...
		function userData(posts) {
			for (var i = 0, ii = posts.length; i < ii; i++) {
				if (posts[i].index || posts[i].index === 0) {
					...
					$('[component="post/replies"] [component="post"][data-pid="'+posts[i].pid+'"]').data('usercard', posts[i].user);
				}
			}
		}
		...
		socket.on('event:voted',function(data){
				...
				$('[component="post"][data-pid="'+data.post.pid+'"]').each(function(){
					var el=$(this).find('.ro-toggle-vote:first > span');
